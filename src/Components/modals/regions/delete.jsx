import React from 'react';
import { useMutation, useQueryClient } from 'react-query';

import Modal from 'antd/es/modal/Modal';
import { useAppStore } from '../../../store/store';
import swal from 'sweetalert';

import { deleteRegion } from '../../../http/regions';
import { notification } from 'antd';

const DeleteRegion = () => {
  const queryClient = useQueryClient();
  const showRegionsDeleteModal = useAppStore(
    (state) => state.showRegionsDeleteModal
  );

  const toggleRegionsDeleteModal = useAppStore(
    (state) => state.toggleRegionsDeleteModal
  );

  const [api, contextHolder] = notification.useNotification();
  const openNotification = (type, message, description) => {
    api[type]({
      message,
      description,
    });
  };

  const selectedRecord = useAppStore((state) => state.selectedRecord);

  const { mutate, isLoading } = useMutation(
    ['deleteRegion', selectedRecord.id],
    () => deleteRegion(selectedRecord.id),
    {
      onSuccess: () => {
        !isLoading && toggleRegionsDeleteModal();
        queryClient.invalidateQueries({ queryKey: ['district'] });
        swal({
          text: 'Region deleted successfully',
          title: 'Sucess!',
          icon: 'success',
        });
      },
      onError: (err) => {
        openNotification('error', 'Error', err.message);
      },
    }
  );

  const handleDelete = (id) => {
    try {
      if (id) {
        mutate(id);
      }
    } catch (err) {
      console.log(err);
    }
  };

  return (
    <div className="font-body">
      {contextHolder}
      <Modal
        open={showRegionsDeleteModal}
        onOk={() => handleDelete(selectedRecord && selectedRecord.id)}
        onCancel={toggleRegionsDeleteModal}
        maskClosable={false}
      >
        <div className="flex flex-col gap-2">
          <h2 className="text-2xl"> Deleting this record is irreverisble</h2>
          <h3>Are you sure you want to delete?</h3>
        </div>
      </Modal>
    </div>
  );
};

export default DeleteRegion;
