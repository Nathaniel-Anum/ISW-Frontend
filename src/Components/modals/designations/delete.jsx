...
const DeleteDesignation = () => {
  ...
  const showDesignationsDeleteModal = useAppStore(
    (state) => state.showDesignationsDeleteModal
  );

  const toggleDesignationsDeleteModal = useAppStore(
    (state) => state.toggleDesignationsDeleteModal
  );
  ...
  const { mutate, isLoading } = useMutation(
    ...
    () => deleteDesignation(selectedRecord.id),
    {
      onSuccess: () => {
        !isLoading && toggleDesignationsDeleteModal();
  ...
  return (
    <div className="font-body">
      {contextHolder}
      <Modal
        open={showDesignationsDeleteModal}
        onOk={() => handleDelete(selectedRecord && selectedRecord.id)}
        onCancel={toggleDesignationsDeleteModal}
        maskClosable={false}
      >
