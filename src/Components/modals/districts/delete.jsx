...
import { deleteDistrict } from '../../../http/districts';
import { notification } from 'antd';

const DeleteDistrict = () => {
  const queryClient = useQueryClient();
  const showDistrictDeleteModal = useAppStore(
    (state) => state.showDistrictDeleteModal
  );

  const toggleDistrictDeleteModal = useAppStore(
    (state) => state.toggleDistrictDeleteModal
  );
  ...
  const { mutate, isLoading } = useMutation(
    ...
    () => deleteDistrict(selectedRecord.id),
    {
      onSuccess: () => {
        !isLoading && toggleDistrictDeleteModal();
  ...
  return (
    <div className="font-body">
      ...
      <Modal
        ...
        onCancel={toggleDistrictDeleteModal}
        maskClosable={false}
      >
