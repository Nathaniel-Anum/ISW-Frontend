...
export const login = async (staffId: string, password: string) => {
  ...
  localStorage.setItem('mustResetPassword', response.data.mustResetPassword.toString());
...
export const resetPasswordWithToken = async (token: string, newPassword: string) => {
  const response = await api.patch('/auth/reset-password-with-token', { token, newPassword });
  return response.data;
};
